...
cmd.hear(/^(?:отмена|cancel|отменить|отмени|верни)\s(2)$/i, async (message, bot) => {
    if (message.chat.type === 2) {
        ...
        if (message.chat.games[message.chat.games.length - 1].canceledBets?.some(x => x.id === message.senderId && x.type === 2)) {
            return bot(`Вы уже отменяли ставку на x2 в этом раунде.`);
        }
        ...
        return bot(`Ваша ставка на x2 в размере ${utils.sp(lastBet.amount)} ${currency} была отменена. Вам возвращено ${utils.sp(refundAmount)} ${currency}.`);
    }
});
