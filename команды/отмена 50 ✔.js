...
cmd.hear(/^(?:отмена|cancel|отменить|отмени|верни)\s(50)$/i, async (message, bot) => {
    if (message.chat.type === 2) {
        ...
        if (message.chat.games[message.chat.games.length - 1].canceledBets?.some(x => x.id === message.senderId && x.type === 50)) {
            return bot(`Вы уже отменяли ставку на x50 в этом раунде.`);
        }
        ...
        message.chat.games[message.chat.games.length - 1].stavki = 
            message.chat.games[message.chat.games.length - 1].stavki.filter(x => x.id !== message.senderId || x.type !== 50);
